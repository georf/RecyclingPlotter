...
// Stepper definitions
Stepper penStepper(100, PEN_PIN1, PEN_PIN2, PEN_PIN3, PEN_PIN4);
Stepper xStepper(120, AXIS_X_PIN1, AXIS_X_PIN2, AXIS_X_PIN3, AXIS_X_PIN4);
Stepper yStepper(120, AXIS_Y_PIN1, AXIS_Y_PIN2, AXIS_Y_PIN3, AXIS_Y_PIN4);

// Hack to adjust bad gearbox
bool lastYUp = true;
...
void setup() {
  ...
  // ----- define normal stepper speed
  penStepper.setSpeed(80);
  xStepper.setSpeed(60);
  yStepper.setSpeed(60);

  // ----- show menu on serial device
  menu();
}
...
void loop() {
  ...
  while (Serial.available()) {
    ...
    if (INPUT_CHAR == '\n') {          // check for line feed
      ...
      memset(BUFFER, '\0',
             sizeof(BUFFER));  // fill buffer with string terminators
...
void menu() {
  ...
  Serial.println(F("    T5 ................. adjust pen"));
  Serial.println(F("    T6 ................. test pattern: ABC"));
  Serial.println(F("    T7 ................. test pattern: target"));
  Serial.println(F("    T8 ................. test pattern: radials"));
  Serial.println(F("  ------------------------------------------------------"));
}
...
void process(String string) {
  ...
  if (INPUT_STRING.startsWith("T2")) {
    ...
    if (!(START < 0)) {
      ...
      Serial.print(F("Drawing now "));
      Serial.print(SCALE_FACTOR * 100);
      Serial.println(F("%"));
    } else {
  ...
  if (INPUT_STRING.startsWith("T5")) {
    ...
    Serial.println(F(""));
    Serial.println(F("  ----------------------------------------------"));
    Serial.println(F("    Position the pen to the paper"));
    Serial.println(F("  ----------------------------------------------"));
    Serial.println(F("    'U'  ^"));
    Serial.println(F("         |"));
    Serial.println(F("    'N'  v"));
    Serial.println(F("    Exit = 'E'"));

    // ----- flush the buffer
    while (Serial.available() > 0) Serial.read();

    // ----- control motors with 'U' and 'N' keys
...
void draw_line(int x1, int y1, int x2,
               int y2) {  // these are "scaled" co-ordinates

  // ----- locals
  int x = x1,  // current "scaled" X-axis position
  ...
  slope = (shortest
           << 1);  // multiply by two ... slope equals (shortest*2/longest*2)
  ...
  */
  ...
  /*
     The current X-axis and Y-axis positions will now be incremented
     (decremented) each time through the loop. These intermediate steps are
     parsed to the plot(x,y) function which calculates the number of steps
     required to reach each of these intermediate co-ordinates. This effectively
...
//--------- -----------------------------------------------------------
void left() { xStepper.step(-1); }

//--------------------------------------------------------------------
// RIGHT()           (move the pen 1 step right)
//--------- -----------------------------------------------------------
void right() { xStepper.step(1); }
...
void draw_arc_cw(float x, float y, float i, float j) {
  ...
  if ((i < -100) || (i > 100) || (j < -100) || (j > 100)) {
    ...
    // ----- variables
    float thisX = LAST_X / (STEPS_PER_MM *
                            SCALE_FACTOR),  // current unscaled X co-ordinate
        thisY = LAST_Y / (STEPS_PER_MM *
                          SCALE_FACTOR),  // current unscaled Y co-ordinate
    ...
    */
    ...
    if (current_angle <= 0)
      current_angle += 2 * PI;  // angles now 360..0 degrees CW
...
/*
   ...
   The arc length may now be calculated using the formula arc_length =
   radius*angle.
*/
void draw_arc_ccw(float x, float y, float i, float j) {
  ...
  if ((i < -100) || (i > 100) || (j < -100) || (j > 100)) {
    ...
    // ----- variables
    float thisX = LAST_X / SCALE_FACTOR,  // current unscaled X co-ordinate
    ...
    */
    ...
    for (int segment = 1; segment < segments; segment++) {
      ...
      if (next_angle > 2 * PI)
        next_angle -= 2 * PI;                     // check if angle crosses zero
      newX = circleX + radius * cos(next_angle);  // standard circle formula
      newY = circleY + radius * sin(next_angle);
      move_to(newX, newY);
    }
...
//---------------------------------------------------------------------------
// STEPPERS_OFF
// Take voltage from x and y steppers to give more power to pen stepper
//---------------------------------------------------------------------------
void steppersOff() {
  digitalWrite(AXIS_Y_PIN1, LOW);
  digitalWrite(AXIS_Y_PIN2, LOW);
  digitalWrite(AXIS_Y_PIN3, LOW);
  digitalWrite(AXIS_Y_PIN4, LOW);

  digitalWrite(AXIS_X_PIN1, LOW);
  digitalWrite(AXIS_X_PIN2, LOW);
  digitalWrite(AXIS_X_PIN3, LOW);
  digitalWrite(AXIS_X_PIN4, LOW);
}
